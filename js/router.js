/* ═══════════════════════════════════════════════════
   ...
   Changes:
   - Removed 'java' page (merged into Knowledge Base)
   - Added 'resources' page (replaces videos)
   - Kept 'videos' route alias → resources for any old links
   - refreshBadges updated to match new nav badges
═══════════════════════════════════════════════════ */
window.Router = (() => {
  ...
  const PAGES = {
    ...
    videos:     () => window.Pages.Resources.render(),   // alias
  ...
  function goto(page) {
    ...
    current = page === 'videos' ? 'resources' : page; // normalise alias

    document.querySelectorAll('.page').forEach(p => p.classList.remove('active'));
    document.querySelectorAll('.nav-item[data-page]').forEach(b => b.classList.remove('active'));

    const pg  = document.getElementById('page-' + (page === 'videos' ? 'resources' : page));
    const btn = document.querySelector(`.nav-item[data-page="${page === 'videos' ? 'resources' : page}"]`);
    if (pg)  pg.classList.add('active');
    if (btn) btn.classList.add('active');

    try { PAGES[page](); } catch(e) { console.error('[Router] Error:', page, e); }
